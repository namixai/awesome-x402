...
- [Usenami](https://agentic.market/services/api-usenami-io) - Multi-venue perpetual-futures market data for arb-aware AI agents. 25 pay-per-call endpoints on Base mainnet ($0.001–$0.05 USDC), spanning a cheap raw-data tier and a premium decision-ready SIGNAL tier. Flagship: a **decision-ready funding-arb signal** (`/v1/perp/arbitrage/funding-signal`, $0.025) — cross-venue funding spread net of round-trip taker fees, depth-sized max notional ($1K/$5K/$10K), and funding-periods-to-breakeven, ranked — plus a **complete 7-day hourly backtest dataset** (`/v1/backtest/data`, $0.05) for agent strategy backtesting. Also: sized-depth slippage quotes (`/v1/orderbook/depth/aggregate/{venue}/{symbol}`, $0.010), multi-venue execution routing (`/v1/orderbook/multi-venue-execution`, $0.010), OI-delta / volume-anomaly / funding-flip regime signals ($0.003), and oracle price-source family classification for basis-risk (`/v1/perp/oracle-families`, $0.001). Entry tier from $0.001: per-venue funding rates across 30+ exchanges (`/v1/funding/current`), cross-venue funding spread (`/v1/perp/funding-spread?ticker=BTC`), and RWA perpetual coverage on Hyperliquid HIP-3 DEXes — stocks, metals, forex, commodities, pre-IPO synthetics (`/v1/rwa/perp-coverage`). Free preview tier (no payment) at `/v1/preview/*`. Differentiated vs CoinGecko/CMC by perp-first focus + RWA inclusion + backtestable depth. x402 v2 with Bazaar discovery extension via Coinbase CDP Facilitator. MCP server (12 tools) at [namixai/funding-mcp](https://github.com/namixai/funding-mcp). ([API base](https://api.usenami.io)) ([Provider](https://usenami.io))
